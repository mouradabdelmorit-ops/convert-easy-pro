...
import { ArrowRight, Download, Loader2, X, CheckCircle, Music } from "lucide-react";
import { audioConversions, type ConversionTool } from "@/data/conversionTools";
...
const AudioToolPage = () => {
  ...
  const tool = audioConversions.find(t => t.slug === slug);
  ...
  const handleConversion = async () => {
    ...
    for (let index = 0; index < files.length; index++) {
      ...
      try {
        ...
        formData.append('targetFormat', tool.toFormat || 'MP3');
  ...
  if (!tool) {
    return <Navigate to="/audio-converter" replace />;
  }

  return (
    <>
      <ToolPageSEO tool={tool} categoryPath="audio" />
      ...
      <div className="min-h-screen bg-background">
        ...
        <main className="pt-20">
            ...
            <div className="container mx-auto px-4 relative z-10">
              <div className="max-w-3xl mx-auto text-center">
                <nav className="text-sm text-muted-foreground mb-4">
                  ...
                  <a href="/audio-converter" className="hover:text-foreground">Audio Tools</a>
                  <span className="mx-2">/</span>
                ...
                <div className={`rounded-2xl border-2 border-dashed transition-all duration-300 ${
                  ...
                  <div className="p-6 md:p-8 text-center">
                    <div className={`w-14 h-14 md:w-16 md:h-16 mx-auto mb-4 rounded-xl flex items-center justify-center ${
                      ...
                      <Music className={`w-7 h-7 md:w-8 md:h-8 ${isDragActive ? 'text-white' : 'text-green-400'}`} />
                    </div>
                    <h3 className="text-lg md:text-xl font-semibold text-foreground mb-2">
                      {isDragActive ? 'Drop your files here!' : `Upload ${tool.fromFormat || 'Audio'} Files`}
                    </h3>
                    <p className="text-muted-foreground mb-4 text-sm md:text-base">
                      Drag & drop or click to browse • Max 200MB per file
                    </p>
                    <Button onClick={open} className="bg-green-500 hover:bg-green-600 text-white" size="lg">
                    ...
                    <div key={index} className="glass rounded-xl p-4 flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-muted-foreground">
                          ...
                          {tool.toFormat && <span className="text-green-400 ml-2">→ {tool.toFormat}</span>}
                        </p>
                        {f.status === "converting" && (
          ...
          {/* Internal Links */}
          <InternalLinks toolId={tool.id} categoryPath="audio" />
        </main>
